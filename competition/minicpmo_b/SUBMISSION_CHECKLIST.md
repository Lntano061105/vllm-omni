...
| 最终确定性归档 | 未生成 | 全证据通过后运行 `build_final_submission.py`；隐藏候选包经流式内部 SHA256、metadata 与当前权威证据树逐文件对照后原子发布，失败不覆盖旧包；再用 `validate_final_evidence.py --require-package` 独立重算归档、sidecar、成员全集和内容完整性 |
